...
def send_new_epochs_to_training_process(socket, res_idx):
    ...
    try:
        if res_idx == -1:
            ...
            encoded = (
                    res["Epochs"].__str__() + ',' + res["Sample Size"].__str__() + ',' + res["Conv"].__str__() + ',' + \
                    res["Pool"].__str__() + ',' + res["Dense"].__str__() + ',' + res["LSTM"].__str__() + ',' + \
                    res["GRU"].__str__() + ',' + res["RNN"].__str__())
...
def create_model(layers_lst, layer2add='dense'):
    """
    This fucntion creates a local model of the nn, used to render the image representing it

    :param layers_lst: list of layer types used in the model
    :param layer2add: last layer of the model
...
def create_layers_lst(conv_number, pool_number, lstm_number, gru_number, rnn_number, dense_number):
    """
    Use this function to construct the list of strs representing each layer.
    ...
    :return: the list of strs representing each layer
    """
    layers_lst = []
    if conv_number > 0:
...
def send_sbto_run_signal():
    ...
    if st.session_state.run_signal and st.session_state.initialized:
        ...
        try:
            ...
            print("An exception occurred on send sbto run signal:", error)  # An exception occurred: division by zero
...
def socket_listener_sbto(server_socket):
    ...
    while True:
        try:
            ...
            print("An exception occurred on socket listener sbto:", error)  # An exception occurred: division by zero
            print('Disconnecting')
            conn, addr = server_socket.accept()
            print("Connected by", addr)
    return sbto_res_df
...
def socket_listener_live_sbto(socket):
    ...
    while True:
        try:
            ...
            print("An exception occurred live sbto:", error)
            print('Disconnecting live sbto socket')
            disconnect_socket(conn)
            conn, addr = socket.accept()
            print("Connected by live sbto", addr)
...
def connect_sockets():
    ...
    This function:
    1. Loads socket configuration from `config_video.json`.
...
if 'stream_batch_train' not in st.session_state:
    st.session_state['stream_batch_train'] = 1002  # Default value
if 'stream_batch_test' not in st.session_state:
    st.session_state['stream_batch_test'] = 334  # Default value
if 'num_of_conv_layers' not in st.session_state:
    st.session_state['num_of_conv_layers'] = 2  # Default value
...
if 'num_of_dense_layers' not in st.session_state:
    st.session_state['num_of_dense_layers'] = 1  # Default value
...
if 'size_of_batch' not in st.session_state:
    st.session_state['size_of_batch'] = 10  # Default value
if 'bo_data_train' not in st.session_state:
    st.session_state['bo_data_train'] = 190  # Default value
if 'bo_data_test' not in st.session_state:
    st.session_state['bo_data_test'] = 63  # Default value
if 'sample_size_low' not in st.session_state:
    st.session_state['sample_size_low'] = 0.90  # Default value
if 'sample_size_high' not in st.session_state:
    st.session_state['sample_size_high'] = 0.50  # Default value
...
if 'num_of_conv_layers_high' not in st.session_state:
    st.session_state['num_of_conv_layers_high'] = 3  # Default value
...
if 'num_of_pool_layers_high' not in st.session_state:
    st.session_state['num_of_pool_layers_high'] = 3  # Default value
...
if 'num_of_dense_layers_high' not in st.session_state:
    st.session_state['num_of_dense_layers_high'] = 3  # Default value
...
if 'num_of_epochs_low' not in st.session_state:
    st.session_state['num_of_epochs_low'] = 3  # Default value
...
if 'lamda_acc' not in st.session_state:
    st.session_state['lamda_acc'] = 0.5  # Default value
...
with st.sidebar:
    ...
    cols = st.columns(2)
    with cols[0]:
        stream_batch_train = st.number_input("Stream Train Size:", step=1, value=1002, key='stream_batch_train',
                                             on_change=update_config_file('stream_batch_train'))
    with cols[1]:
        stream_batch_test = st.number_input("Stream Test Size:", step=1, value=334, key='stream_batch_test',
                                            on_change=update_config_file('stream_batch_test'))
    num_of_conv_layers = st.number_input("Default Number of Conv Layers:", step=1, value=2, key='num_of_conv_layers',
    ...
    num_of_dense_layers = st.number_input("Default Number of Dense Layers:", step=1, value=1, key='num_of_dense_layers',
    ...
    with cols[1]:
        size_of_batch = st.number_input("Batch Size:", value=10, step=10, min_value=1, max_value=stream_batch_train,
                                        key='size_of_batch', on_change=update_config_file('size_of_batch'))

    st.header("SuBiTO:")
    cols = st.columns(2)
    with cols[0]:
        bo_data_train = st.number_input("SuBiTO Train Size:", step=1, value=190, key='bo_data_train',
                                        on_change=update_config_file('bo_data_train'))
    with cols[1]:
        bo_data_test = st.number_input("SuBiTO Test Size:", step=1, value=63, key='bo_data_test',
                                       on_change=update_config_file('bo_data_test'))

    cols = st.columns(2)
    with cols[0]:
        sample_size_high = st.number_input("Compression R. Low:", step=0.05, value=0.50, min_value=0.0, max_value=1.0,
                                           key='sample_size_high', on_change=update_config_file('sample_size_high'))
    with cols[1]:
        sample_size_low = st.number_input("Compression R. High:", step=0.05, value=0.90, min_value=sample_size_high,
                                          max_value=1.0,
                                          key='sample_size_low', on_change=update_config_file('sample_size_low'))
    cols = st.columns(2)
    with cols[0]:
        num_of_conv_layers_low = st.number_input('Conv Low:', step=1, value=1, min_value=1,
                                                 key='num_of_conv_layers_low',
                                                 on_change=update_config_file('num_of_conv_layers_low'))
    with cols[1]:
        num_of_conv_layers_high = st.number_input('Conv High:', step=1, value=3, min_value=num_of_conv_layers_low,
                                                  key='num_of_conv_layers_high',
                                                  on_change=update_config_file('num_of_conv_layers_high'))

    cols = st.columns(2)
    with cols[0]:
        num_of_pool_layers_low = st.number_input('Pool Low:', step=1, value=0, min_value=0,
                                                 key='num_of_pool_layers_low',
                                                 on_change=update_config_file('num_of_pool_layers_low'))
    with cols[1]:
        num_of_pool_layers_high = st.number_input('Pool High:', step=1, value=3, min_value=num_of_pool_layers_low,
                                                  key='num_of_pool_layers_high',
                                                  on_change=update_config_file('num_of_pool_layers_high'))

    cols = st.columns(2)
    with cols[0]:
        num_of_dense_layers_low = st.number_input('Dense Low:', step=1, value=1, min_value=0,
                                                  key='num_of_dense_layers_low',
                                                  on_change=update_config_file('num_of_dense_layers_low'))
    with cols[1]:
        num_of_dense_layers_high = st.number_input('Dense High:', step=1, value=3, min_value=num_of_dense_layers_low,
    ...
    with cols[0]:
        num_of_lstm_layers_low = st.number_input('LSTM Low:', step=1, value=0, min_value=0,
                                                 ...
                                                 on_change=update_config_file('num_of_lstm_layers_low'))
    with cols[1]:
        num_of_lstm_layers_high = st.number_input('LSTM High:', step=1, value=2, min_value=0,
                                                  key='num_of_lstm_layers_high',
                                                  on_change=update_config_file('num_of_lstm_layers_high'))

    cols = st.columns(2)
    with cols[0]:
        num_of_gru_layers_low = st.number_input('GRU Low:', step=1, value=0, min_value=0, key='num_of_gru_layers_low',
                                                on_change=update_config_file('num_of_gru_layers_low'))
    with cols[1]:
        num_of_gru_layers_high = st.number_input('GRU High:', step=1, value=2, min_value=0,
                                                 key='num_of_gru_layers_high',
                                                 on_change=update_config_file('num_of_gru_layers_high'))

    cols = st.columns(2)
    with cols[0]:
        num_of_rnn_layers_low = st.number_input('RNN Low:', step=1, value=0, min_value=0, key='num_of_rnn_layers_low',
                                                on_change=update_config_file('num_of_rnn_layers_low'))
    with cols[1]:
        num_of_rnn_layers_high = st.number_input('RNN High:', step=1, value=2, min_value=0,
                                                 key='num_of_rnn_layers_high',
                                                 on_change=update_config_file('num_of_rnn_layers_high'))
    ...
    cols = st.columns(2)
    with cols[0]:
        num_of_epochs_low = st.number_input('Epochs Low:', step=1, value=3, min_value=1, key='num_of_epochs_low',
    ...
    lamda_acc = st.number_input('Lambda Accuracy:', step=0.1, value=0.5, min_value=0.01, max_value=1.0, key='lamda_acc',
...
with sbt:
    ...
    if st.session_state['subito_running'] is False:
        ...
        with cols[0]:
            if 'bo_res' in st.session_state:
                st.metric("Compression Ratio", round((100 - st.session_state['sr1']), 1).__str__() + '%')
        ...
        with cols[1]:
            if 'bo_res' in st.session_state:
                st.metric("Compression Ratio", round((100 - st.session_state['sr2']), 1).__str__() + '%')
        ...
        with cols[2]:
            if 'bo_res' in st.session_state:
                st.metric("Compression Ratio", round((100 - st.session_state['sr3']), 1).__str__() + '%')
...
with prod_pipe:
    ...
    if genre == "SuBiTO Mode:rocket:":
        st.session_state['manual'] = False
        with cols[1]:
            st.metric("Compression Ratio", round((100 - st.session_state['sr']), 1).__str__() + '%')
        with cols[2]:
