...
def layered_view(model,
    ...
    for index, layer in enumerate(model.layers):
        ...
        if type(layer) == SpacingDummyLayer:
            ...
            continue
        if hasattr(layer, "layer") and isinstance(layer.layer, Layer):
            # If the layer is TimeDistributed, add the nested layer directly
            layer_type = type(layer.layer)
        else:
        ...
        if hasattr(layer, 'output_shape'):
            if hasattr(layer, "layer") and isinstance(layer.layer, Layer):
                # If the layer is TimeDistributed, add the nested layer directly
                # output_shape = (None, layer.output_shape[1], layer.output_shape[2], layer.output_shape[3] * layer.output_shape[0])
                output_shape = layer.output_shape[1:]
            elif hasattr(layer, "return_sequences") and layer.return_sequences:
                output_shape = layer.output_shape[1:]
            else:
