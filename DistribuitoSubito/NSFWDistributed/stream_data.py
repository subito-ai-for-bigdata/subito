...
def load_dataset(train_dir, test_dir):
    """
    Loads the dataset from train and test directories, assigning incremental labels to each class.
    ...
    Returns:
        tuple: ((x_train, y_train), (x_test, y_test)), class_labels
            - x_train, x_test: NumPy arrays of image data (normalized to [0, 1]).
            - y_train, y_test: NumPy arrays of corresponding labels.
...
def shuffle_dataset(x, y):
    """
    Shuffles the dataset while maintaining correspondence between data and labels.

    Args:
        x (np.ndarray): Feature data.
        y (np.ndarray): Labels corresponding to the data.
