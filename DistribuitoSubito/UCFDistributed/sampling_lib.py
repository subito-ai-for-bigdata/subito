...
def sampling_method(sampling_method_id, received_images_reshaped, received_labels_decoded, sample_size, n_workers=1, cluster=None):
    ...
    if sampling_method_id == 0:
        ...
        divided_by = 1
        received_labels_decoded = np.array(received_labels_decoded)
        received_labels_decoded = received_labels_decoded.reshape(len(received_labels_decoded), 1)
        #TODO
        # compute the
        received_images_reshaped = da.from_array(received_images_reshaped, chunks=(
            len(received_images_reshaped) // divided_by, 20, 64, 64, 3))  # You can adjust the chunk size as needed
        received_labels_decoded = da.from_array(received_labels_decoded, chunks=(
            len(received_labels_decoded) // divided_by, 1))  # You can adjust the chunk size as needed
